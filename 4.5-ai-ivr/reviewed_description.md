...
2. **Live call patching** — the new idea in this tutorial. While the call is
...
follows further down. There is a lot of setup here — service config, an `wss://`
tunnel, a relay server, and an LLM key — so do the steps **in order**; each one
depends on the previous.

```bash
# 0. From the tutorial folder
cd 4.5-ai-ivr
cp .env.example .env
...
> server still fails fast with a clear message if a required key is missing — but
> make sure `WS_ENDPOINT` is **`wss://`** (Sinch requires TLS for Voice Relay),
> not a plain `ws://` URL.
...
setup is a **STATIC** call behavior on the service — no webhook server needed
...
| **Ambiguous intent** | A reply that isn't a known one-word route is spoken back verbatim as a clarifying question, so the caller can refine — no silent mis-route. |
